...
def mock_obstacles_update(num_dynamic_obstacles: int = 4, num_static_obstacles: int = 2):
    """mocks an obstacles update"""
    if (num_static_obstacles < 0) or (num_static_obstacles < 0):
        raise ValueError("invalid obstacle number please enusre obstacles number is greater than or equal to 0")
    ...
    obstacles_update = _mock_dynamic_obstacles(obstacle_update=obstacles_update, num_dynamic_obstacles=num_dynamic_obstacles)
    obstacles_update = _mock_static_obstacles(obstacles_update=obstacles_update, num_static_obstacles=num_static_obstacles)

    return obstacles_update

def _mock_dynamic_obstacles(agent_position: List[float, float, float], obstacles_update: ObstaclesUpdate, detection_range: float = 1000, num_dynamic_obstacles: int = 5) -> ObstaclesUpdate:
    ...
    for x in range(0,num_dynamic_obstacles):
        ...
        obstacles_update.dynamic_obstacles[x].id.tag = f"{obstacle_class}_{obstacle_type.name}_{x}"
        ...
        obstacles_update.dynamic_obstacles[x].geometry.acceptance_radius = float(random.uniform(float(1.2), float(5)))
        ...
        for i in range(0, random_polyshape_vertices.size):
            ...
            obstacles_update.dynamic_obstacles[x].geometry.polyshape_points[i-1].position.x = random_polyshape_vertices[0]
            obstacles_update.dynamic_obstacles[x].geometry.polyshape_points[i-1].position.y = random_polyshape_vertices[1]
            obstacles_update.dynamic_obstacles[x].geometry.polyshape_points[i-1].position.z = 0
            # TODO: Need to get rid of orientation for polyshape vertices.
            obstacles_update.dynamic_obstacles[x].geometry.polyshape_points[i-1].orientation.x = 0
            obstacles_update.dynamic_obstacles[x].geometry.polyshape_points[i-1].orientation.y = 0
            obstacles_update.dynamic_obstacles[x].geometry.polyshape_points[i-1].orientation.z = 0
            obstacles_update.dynamic_obstacles[x].geometry.polyshape_points[i-1].orientation.w = 1

        obstacles_update.dynamic_obstacles[x].state.velocity = random.uniform(float(15), float(30))
        obstacles_update.dynamic_obstacles[x].state.yaw_rate = random.uniform(float(0), float(2.0))

    return obstacles_update 

def _mock_static_obstacles(obstacles_update: ObstaclesUpdate, num_static_obstacles: int = 5) -> ObstaclesUpdate:
...
def _random_polyshape(min_vertices: int = 3, max_vertices: int = 15):
    ...
    if not polygon.is_valid or polygon.is_empty:
        return _random_polyshape(min_vertices, max_vertices)  # Recursively regenerate if invalid
...
def _random_position(position:List[float, float, float], range: float) -> List[float, float, float]:
